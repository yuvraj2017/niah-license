...
class niah_advisor_scan_tags():
    ...
    def get_github_repo_info(self, github_url):
        ...
        if response.status_code == 200:
            ...
            github_info = {
                ...
                "LastCommit": str(last_commit).split("T")[0],
                "Forks": forks,
    ...
    def scan_pypi(self, ecosystem, package, tags):
    ...
    def scan_ruby(self, ecosystem, package, tags):
        ...
        sbom_name = "/mnt/niahdb/niah-advisor/niah_pack/%s/%s/sbom/%s_%s_sbom.json" % (ecosystem, p_name, p_name, tags)
        repo = str(homeurl).split("/")[-1]
        try:

            os.chdir("repos")

            cmd = "GIT_ASKPASS=echo git clone -b %s %s.git" % (tags, homeurl)
            print(cmd)
            status, output = getstatusoutput(cmd)
    ...
    def scan_npm(self, ecosystem, package, tags):
    ...
    def scan_hex(self, ecosystem, package, tags):
        ...
        try:
            ...
            cmd = "GIT_ASKPASS=echo git clone -b %s %s.git" % (tags, g_url)
            print(cmd)
            status, output = getstatusoutput(cmd)
    ...
    def scan_nuget(self, ecosystem, package, tags):
        ...
        try:
            ...
            try:
                ...
                os.chdir("repos")
                sbom_name = "/mnt/niahdb/niah-advisor/niah_pack/%s/%s/sbom/%s_%s_sbom.json" % (ecosystem, p_name, p_name, tags)
                cmd = "GIT_ASKPASS=echo git clone -b %s %s.git" % (tags, source_repo)
                print(cmd)
                status, output = getstatusoutput(cmd)
                print(os.listdir())
    ...
    def scan_pub(self, ecosystem, package, tags):
    ...
    def scan_crates(self, ecosystem, package, tags):
        ...
        try:
            ...
            os.chdir("repos")
            sbom_name = "/mnt/niahdb/niah-advisor/niah_pack/%s/%s/sbom/%s_%s_sbom.json" % (ecosystem, package, package, tags)
            cmd = "GIT_ASKPASS=echo git clone -b %s %s.git" % (tags, git_repo)
            print(cmd)
            status, output = getstatusoutput(cmd)
            print(os.listdir())
    ...
    def scan_composer(self, ecosystem, package, tags):
        ...
        try:
            ...
            print(file_name)
            sbom_name = "/mnt/niahdb/niah-advisor/niah_pack/%s/%s/%s_%s_sbom.json" % (package, p_name, p_name, tags)
    ...
    def get_pack_details(self, ecosystem, package, tags):
        
        if ecosystem == "pypi":
            pack_datails = self.scan_pypi(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "ruby":
            pack_datails = self.scan_ruby(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "npm":
            pack_datails = self.scan_npm(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "hex":
            pack_datails = self.scan_hex(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "nuget":
            pack_datails = self.scan_nuget(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "pub":
            pack_datails = self.scan_pub(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "crates":
            pack_datails = self.scan_crates(ecosystem, package, tags)
            return pack_datails

        if ecosystem == "composer":
            pack_datails = self.scan_composer(ecosystem, package, tags)
            return pack_datails
...
if __name__ == "__main__":
    ...
    res = niah_advisor_scan_tags()
    ecosystem = "pypi"
    package = "urllib3"

    res.get_pack_details(ecosystem, package, tags)
