...
class get_json_feeds():
    ...
    def get_echosystem_vuln(self, echosystem):
        ...
        echosystem_platforms = ['ubuntu', 'debian', 'rhel', 'oracle']
    ...
    def get_package_vuln(self, package, echosystem):
        ...
        if echosystem not in echosystem and echosystem not in echosystems:
            return False
        ...
        if echosystem in echosystem_platforms:
            ...
            jsondata1 = list(filter(lambda x: (package == x['package']), jsondata1["results"]))

            with open("/var/DB/feeds/non-cve/noncve_feed.json", "r") as f:
                jsondata2 = json.load(f)
            
            try:
                jsondata2 = list(filter(lambda x:(package == x['product']),jsondata2['results']))
            except:
                jsondata2 = []
    ...
    def get_package_details(self, package, echosystem, check=False):
        results = {}
        # print(osname)
        print("111EEE", echosystem)
        print("222PPP", package)
        ...
        if echosystem == "ubuntu":
            ...
            if os.path.isfile("/mnt/niahdb/packagesdb/composer/%s.json" % package):
                ...
                # if len(jsondata['current']['author']) > 0:
                #     results['author'] = jsondata['current']['author'][0]['name']
            ...
            if os.path.isfile("/mnt/niahdb/packagesdb/maven/new/%s.json" % package):
                with open("/mnt/niahdb/packagesdb/maven/new/%s.json" % package, "r") as f:
                    jsondata = json.load(f)
            ...
            if os.path.isfile("/mnt/niahdb/packagesdb/ruby/%s.json" % package):
                ...
                try:
                    ...
                    results['home_page'] = homeurl
                    results['license'] = license
                    results['version'] = latest_version
                    results['package_url'] = ""
                    results['requires_dist'] = r_dependencies
                    results['requires_version'] = ""
                    results['summary'] = ""
                    results['releases'] = ""
