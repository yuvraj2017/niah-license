...
class debianParser():
    ...
    def rssfeed(self, platforms):
        for platform in platforms:
            target_dir = "/mnt/niahdb/packagesdb/platforms/debian/%s" % platform
            ...
            with open("/mnt/niahdb/niah-advisor/niah_pack/debian_update.json", "w") as f:
                json.dump(packages_list, f, indent=2)

            # with open("/mnt/niahdb/niah-advisor/niah_pack/debian_update.json", "r") as f:
            ...
            for platform in platform_url:
                ...
                if check:
                    for package in pkgjson:
                        target_dir = '/mnt/niahdb/packagesdb/platforms/debian/%s/%s' % (platform, package)
                        if not os.path.isdir(target_dir):
                            link = "https://packages.debian.org/%s/%s" % (platform, package)
                            print(link)
    ...
    def get_package(self, link, platform, packagename=False):
        ...
        target_dir = '/mnt/niahdb/packagesdb/platforms/debian/%s/%s' % (platform, packagename)
        ...
        if not os.path.exists("/tmp/lic_updates/debian"):
                os.makedirs("/tmp/lic_updates/debian")
        ...
        with open("/tmp/lic_updates/debian/%s.json" % packagename, "w") as outfile:
            json.dump(results, outfile, indent=2)
